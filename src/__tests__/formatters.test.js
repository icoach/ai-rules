...
describe('formatters', () => {
    ...
    rules: [
      {
        ...
        globs: ['*.js', '*.ts'],
        content: 'Use early returns whenever possible to make the code more readable.'
      },
      {
        name: 'Python Rules',
  ...
  describe('toCursorFormat', () => {
    ...
    test('should create cursor rules directory with .mdc files', () => {
      toCursorFormat(testData, testOutputDir)
    ...
    test('should create .mdc files with correct format', () => {
      ...
      const content = fs.readFileSync('test-cursor/rules/General-Coding-Rules.mdc', 'utf8')
    ...
    test('should handle rules without globs', () => {
      ...
      const content = fs.readFileSync('test-cursor/rules/Python-Rules.mdc', 'utf8')
      expect(content).toContain('description: Rules for Python')
      expect(content).not.toContain('globs:')
    })

    test('should create .cursorignore file when ignoreData provided', () => {
      toCursorFormat(testData, testOutputDir, false, testIgnoreData)
    ...
    test('should handle force flag by overwriting existing directory', () => {
      ...
      fs.writeFileSync(path.join(testOutputDir, 'existing.mdc'), 'existing content')
  ...
  describe('toClaudeFormat', () => {
    ...
    test('should create .claude/settings.json when ignoreData provided', () => {
      toClaudeFormat(testData, testOutputFile, false, testIgnoreData)

      expect(fs.existsSync('.claude/settings.json')).toBe(true)
      const settingsContent = JSON.parse(fs.readFileSync('.claude/settings.json', 'utf8'))
  ...
  describe('toClineFormat', () => {
    ...
    test('should create .aiignore when ignoreData provided', () => {
      toClineFormat(testData, testOutputFile, false, testIgnoreData)
  ...
  describe('toCodexFormat', () => {
    ...
    test('should create .codex/config.json when ignoreData provided', () => {
      toCodexFormat(testData, testOutputFile, false, testIgnoreData)

      expect(fs.existsSync('.codex/config.json')).toBe(true)
      const configContent = JSON.parse(fs.readFileSync('.codex/config.json', 'utf8'))
  ...
  describe('toKiloCodeFormat', () => {
    ...
    test('should create .md files with correct format', () => {
      ...
      const content = fs.readFileSync('test-kilocode/rules/General-Coding-Rules.md', 'utf8')
  ...
  describe('toWindsurfFormat', () => {
    ...
    test('should create .md files with windsurf-specific format', () => {
      ...
      const content = fs.readFileSync('test-windsurf/rules/General-Coding-Rules.md', 'utf8')
  ...
  describe('filename sanitization', () => {
    ...
    test('should sanitize filenames for cursor format', () => {
      ...
      expect(files[0]).toMatch(/Test-Rule-With-SpecialCharacters--Spaces\.mdc/)
    })
    ...
    test('should sanitize filenames for kilocode format', () => {
      ...
      expect(files[0]).toMatch(/Test-Rule-With-SpecialCharacters--Spaces\.md/)
    })
  ...
  describe('directory creation', () => {
    afterEach(() => {
      cleanupFiles(['test-cursor', 'test-kilocode', 'test-windsurf', '.claude', '.codex', 'test-claude.md', 'test-agents.md'])
    })

    test('should create nested directories as needed', () => {
      toCursorFormat(testData, 'test-cursor/rules')
      toKiloCodeFormat(testData, 'test-kilocode/rules')
      toWindsurfFormat(testData, 'test-windsurf/rules')
    ...
    test('should create config directories for ignore files', () => {
      toClaudeFormat(testData, 'test-claude.md', false, testIgnoreData)
      toCodexFormat(testData, 'test-agents.md', false, testIgnoreData)
